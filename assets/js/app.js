...
(function () {
  ...
  if (window.IELTS?.Router?.setHashRoute) {
    ...
    // Require admin code after tab/window switch (students only)
    let needsAdminUnlock = false;
    function requestAdminUnlock(reason) {
      if (isAdminView()) return;

      needsAdminUnlock = true;

      const correct = (R()?.ADMIN_PASSCODE || "");
      if (!correct) return; // if not configured, skip

      try {
        Modal().showModal(
          "Admin code required",
          reason || "Admin code is required to continue.",
          {
            mode: "admincode",
            locked: true,
            submitText: "Unlock",
            onConfirm: (code) => String(code || "") === String(correct),
          }
        );
      } catch {}
    }
    ...
    window.addEventListener("blur", () => {
      // blur fires on tab switch / alt-tab / minimize (best effort)
      requestAdminUnlock("You left the exam window. Please enter the admin code to continue.");
    }, true);
    ...
    document.addEventListener("visibilitychange", () => {
      ...
      if (needsAdminUnlock) {
        requestAdminUnlock("Please enter the admin code to continue.");
      }
    }, true);
